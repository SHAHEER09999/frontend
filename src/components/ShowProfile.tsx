...
const ShowProfile = () => {
  ...
  if (loading) {
    return <div className="min-h-screen flex items-center justify-center bg-gray-50 text-slate-600">Loading profile...</div>;
  }

  if (!profile) {
    return <div className="min-h-screen flex items-center justify-center bg-gray-50 text-slate-600">Profile not found.</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 md:p-12 font-sans text-slate-700">
      <div className="max-w-6xl mx-auto">
        ...
        <button 
          onClick={() => navigate(-1)}
          className="mb-6 flex items-center text-sm font-semibold text-slate-500 hover:text-teal-600 transition-colors"
        >
        ...
        <div className="flex flex-col md:flex-row gap-10">
          ...
          <div className="w-full md:w-5/12 space-y-6">
            <div className="flex justify-between items-center pr-2">
              ...
              <button 
                ...
                className="text-gray-400 hover:text-pink-600 transition-colors flex items-center gap-1 text-sm font-semibold"
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200/80">
              <h3 className="font-bold text-lg mb-3 text-slate-800">About Me</h3>
              ...
              {profile.categories && profile.categories.length > 0 && (
                <div>
                  <h4 className="font-semibold text-sm text-slate-700 mb-2">Categories</h4>
                  <div className="flex flex-wrap gap-2">
                    {profile.categories.map((cat, index) => (
                      <span key={index} className="bg-pink-50 text-pink-700 border border-pink-100 text-xs font-semibold px-3 py-1 rounded-full">
          ...
          <div className="w-full md:w-7/12 space-y-6 md:mt-[3.25rem]">
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-200/80 space-y-6">
            ...
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-200/80 space-y-6">
              <div className="text-center mb-6">
                <p className="text-gray-400 font-medium mb-2">Starting Price</p>
              ...
              <div className="border-t border-gray-100 pt-6">
                <h3 className="font-bold text-lg mb-4 text-slate-800">Social Accounts</h3>
                {profile.social_accounts && profile.social_accounts.length > 0 ? (
                  ...
                  <p className="text-sm text-gray-400 italic">No social accounts connected yet.</p>
                )}
              </div>

              <button
                onClick={handleChat}
                className="w-full mt-6 bg-teal-600 hover:bg-teal-700 text-white font-bold py-4 rounded-xl shadow-lg shadow-teal-100 transition-all transform hover:-translate-y-0.5 flex items-center justify-center gap-2"
              >            
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="mt-10 flex flex-col md:flex-row gap-10">
          <div className="w-full md:w-5/12">
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200/80">
              <h3 className="font-bold text-lg mb-3 text-slate-800">Leave Feedback</h3>
              <textarea 
                value={feedbackText}
                onChange={(e) => setFeedbackText(e.target.value)}
                rows={4} 
                className="w-full border border-gray-200 rounded-lg p-3 text-sm focus:ring-2 focus:ring-teal-500 focus:outline-none resize-none mb-3"
                placeholder="Write your review here..."
              ></textarea>
              <button 
                onClick={handleSubmitFeedback}
                className="w-full bg-gray-100 hover:bg-gray-200 text-slate-700 font-semibold text-sm px-4 py-3 rounded-lg transition-colors"
              >
                Submit Feedback
              </button>
            </div>
          ...
          <div className="w-full md:w-7/12">
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-200/80 h-full">
              <h3 className="font-bold text-xl text-slate-900 border-b border-gray-100 pb-4 mb-6">
                Past Feedback
              </h3>
              ...
              {profile.reviews && profile.reviews.length > 0 ? (
                ...
                <div className="flex flex-col items-center justify-center h-40 text-center">
                  <p className="text-gray-400 italic">No feedback has been left yet.</p>
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white p-6 rounded-2xl w-full max-w-md shadow-xl border border-gray-200">
              ...
              <textarea
                ...
                className="w-full border border-gray-200 rounded-lg p-3 text-sm focus:ring-2 focus:ring-pink-500 focus:outline-none resize-none mb-4"
                rows={4}
              />
              ...
              <div className="mb-6">
                ...
                <input
                  ...
                  className="block w-full text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-pink-50 file:text-pink-700 hover:file:bg-pink-100 transition-colors cursor-pointer"
                />

                {reportFiles.length > 0 && (
                  <div className="mt-3 space-y-2">
                    <p className="text-xs font-semibold text-gray-400">Attached Files:</p>
                    <ul className="max-h-32 overflow-y-auto space-y-1">
                      {reportFiles.map((file, index) => (
                        <li key={index} className="flex items-center justify-between bg-gray-50 border border-gray-200 px-3 py-1.5 rounded-md">
              ...
              <div className="flex justify-end gap-3 border-t border-gray-100 pt-4">
                <button
                  ...
                  className="px-4 py-2 bg-gray-100 text-slate-600 font-semibold rounded-lg hover:bg-gray-200 transition-colors"
                >
                  Cancel
                </button>
                <button
                  onClick={submitReport}
                  className="px-4 py-2 bg-pink-600 text-white font-semibold rounded-lg hover:bg-pink-700 transition-colors shadow-md shadow-pink-100"
                >
                  Submit Report
                </button>
              </div>
